...
![Enikki](images/enikki.png)

## 概要

**Enikki（絵日記）** は、Gemini と音声で喋るだけで、今日一日の出来事を「ぼくの夏休み」風の懐かしい絵日記に仕立ててくれるアプリ。入力の面倒さと記録の味気なさを解消し、日々の振り返りを楽しい儀式に変える。

- **対象**: 日記が続かない人、温度感のある記録を残したい人
- **解決する課題**: 「音声対話」と「情緒的な絵と文章」によって振り返りのモチベーション不足改善や手軽さ向上
- **技術的特徴**:
    - **Multimodal Live API**: 遅延のない自然な音声対話
    - **自律制**: AI が状況を判断し、自律的にインタビューを完了
    - **LangGraph**: 品質チェックを含む Agentic な生成ワークフロー
    - **Firestore**: リロード不要のリアルタイム生成反映
...
じゃあ、チェックアウトをテキストの記録ではなく **「今日のイメージ写真」の作成** にしたらどうだろう。1日を1枚の絵にして残す。文字では思い出せなくても、絵を見れば「ああ、あの日か」と蘇る。そういう日報のかたちがあってもいいのではないか。
...
- [hirokts](https://zenn.dev/hirokts)
- [taketaku](https://zenn.dev/taketaku)
